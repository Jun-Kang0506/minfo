...
export function ResultSequence({ answer, index, onBack, onNext, onStartOver, feedback, onFeedback }: { answer: Answer; index: number; onBack: () => void; onNext: () => void; onStartOver: () => void; feedback: FeedbackChoice | null; onFeedback: (choice: FeedbackChoice) => void }) {
  ...
  return (
    <div aria-live="polite">
      <p className={`mb-3 text-[15px] font-bold tabular-nums ${emergency ? "text-danger" : "text-moss"}`}>
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {currentIndex < cards.length - 1 && <button onClick={onNext} className="pressable flex min-h-12 items-center gap-2 rounded-sm bg-moss px-5 text-[16px] font-bold text-white hover:bg-moss-deep">{GUIDED_UI.next[lang]}<IconArrowRight className="h-4 w-4" /></button>}
        </div>
      </div>
    </div>
  );
...
function Summary({ answer, headingRef, emergency }: { answer: Answer; headingRef: React.RefObject<HTMLHeadingElement | null>; emergency: boolean }) {
  const { t } = useLanguage();
  return <><div className="flex flex-wrap items-center justify-between gap-2"><h2 ref={headingRef} tabIndex={-1} className={`text-xs font-bold uppercase tracking-[0.12em] ${emergency ? "text-danger" : "text-moss"}`}>{t.answer.nextStep}</h2>{answer.engine === "claude" && <p className="rounded-sm border border-line bg-paper px-2 py-0.5 text-[11px] font-semibold text-ink-soft">{t.answer.aiBadge}</p>}</div><p className="mt-3 text-[18px] leading-relaxed text-ink">{answer.direct}</p>{answer.safety && <div className="mt-4"><SafetyNotice safety={answer.safety} /></div>}{emergency && <div className="mt-5"><StepsList answer={answer} /></div>}{answer.confidence === "low" && <p className="mt-4 flex items-start gap-2 text-[13.5px] font-semibold text-caution"><IconAlert className="mt-0.5 h-4 w-4 shrink-0" />{t.answer.lowConfidence}</p>}</>;
}

function Steps({ answer, headingRef }: { answer: Answer; headingRef: React.RefObject<HTMLHeadingElement | null> }) { const { t } = useLanguage(); return <><h2 ref={headingRef} tabIndex={-1} className="text-xs font-bold uppercase tracking-[0.12em] text-ink-soft">{t.answer.whatToDo}</h2><StepsList answer={answer} /></>; }
function StepsList({ answer }: { answer: Answer }) { return <ol className="mt-2 divide-y divide-line">{answer.steps.map((step, i) => <li key={i} className="flex gap-3 py-2.5 text-[16px] leading-relaxed text-ink"><span className="w-5 shrink-0 text-right font-bold tabular-nums text-moss">{i + 1}.</span>{step}</li>)}</ol>; }
